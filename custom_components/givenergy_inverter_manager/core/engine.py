...
def accumulate_energy(
    ...
    elif raw.battery_power_w > 0:
        battery_kwh_this_step = (raw.battery_power_w / 1000) * elapsed_h
    ...
    if raw.grid_power_w > 0:
        ...
        # Split into cheap (timed period) vs peak (base rate) for optimization analysis
        ...
        # Fractions are normalised so they always sum to exactly 1.0, preventing
        # overallocation when EV + immersion together exceed house_load_w.
        ...
        if total_frac > 1.0:
            # Normalise proportionally so fracs sum to 1.0
            ev_frac        = ev_raw  / total_frac
            immersion_frac = imm_raw / total_frac
        else:
            ev_frac        = ev_raw
    ...
    # Immersion savings: when the immersion is on and there is solar surplus,
    # the solar kWh diverted saves (import_rate - export_rate) vs the alternative
    # of exporting and later importing. Only count when solar > house load (genuine
    # surplus — otherwise the immersion is importing, not diverting solar).
    if immersion_w > 0:
...
def build_coordinator_data(
    ...
    # ── Copy raw readings into snapshot ───────────────────────────────────────
    ...
    # Apply configured charge target cap (from setup/options form)
    max_target = int(cfg.get(CONF_OVERNIGHT_CHARGE_TARGET, DEFAULT_OVERNIGHT_CHARGE_TARGET))
    if data.charge_decision.target_soc > max_target and not data.charge_decision.skip_charge:
    ...
    if override_immersion is not None:
        ...
        data.should_divert_immersion, data.divert_reason = should_divert_to_immersion(
            ...
            immersion_min_temp=float(cfg.get(
                CONF_IMMERSION_MIN_TEMP, DEFAULT_IMMERSION_MIN_TEMP
            )),
    ...
    data.projected_bill = (
        (data.accrued_bill / max(1, days_in)) * (days_in + days_remaining)
        if days_in > 0 else 0.0
    )
    data.days_in_period = days_in
    data.days_remaining = days_remaining

    # ── Night survival ────────────────────────────────────────────────────────
    # Hours until solar generation resumes. Before 08:00, count forward to 08:00.
    # After 08:00, solar is already available today; count forward to next day's 08:00.
    ...
    (data.will_survive_night,
     ...
     data.survival_reason) = estimate_will_survive_night(
        current_soc=raw.battery_soc,
        battery_capacity_kwh=raw.battery_capacity_kwh,
    ...
    if ev_charger is not None:
        ...
        protect_threshold = float(
            cfg.get(CONF_EV_BATTERY_PROTECT_SOC, DEFAULT_EV_BATTERY_PROTECT_SOC)
        )
