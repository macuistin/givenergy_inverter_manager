...
_S = {
    "table":    "width:100%;border-collapse:collapse;font-family:var(--primary-font-family,sans-serif);font-size:14px;",
    "tr_mid":   "border-bottom:1px solid var(--divider-color,#e0e0e0);",
    "label":    "padding:5px 8px;color:var(--secondary-text-color);font-weight:500;width:38%;vertical-align:top;",
    "value":    "padding:5px 8px;font-weight:600;width:30%;vertical-align:top;",
    "detail":   "padding:5px 8px;color:var(--secondary-text-color);font-size:12px;vertical-align:top;",
    "section":  "padding:8px 8px 2px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--secondary-text-color);",
...
def _row(label: str, value: str, detail: str = "", value_style: str = "") -> str:
    ...
    detail_td = f'<td style="{_S["detail"]}">{detail}</td>' if detail else f'<td style="{_S["detail"]}"></td>'
    return (
...
def build_today_summary_html(data: "CoordinatorData") -> str:
    ...
    rows = [
        ...
        _row("🔋 Battery",  f"{data.battery_soc:.0f}% SoC",
        ...
        _row("Accrued bill",     f"{sym}{data.accrued_bill:.2f}",
...
def build_charge_plan_html(data: "CoordinatorData") -> str:
    ...
    if cd is None:
        return (
            ...
            _row("Status", "No charge decision yet",
                 "Calculated before cheap rate window opens") +
            "</table>"
        )

    decision_str = "✅ Skip charge — solar will cover demand" if cd.skip_charge else f"🔌 Charge to {cd.target_soc}%"
    ...
    rows = [
        ...
        _row("Target SoC",
             ...
             if not cd.skip_charge else "Skipping overnight charge"),
...
def build_week_summary_html(data: "CoordinatorData") -> str:
    ...
    rows = [
        _section("📅 This Week"),
        _row("☀️ Solar",       f"{w.solar_kwh:.1f} kWh",
             _delta(data.today.solar_kwh, y.solar_kwh), _S["highlight"]),
        _row("⬇️ Import",      f"{w.import_kwh:.1f} kWh",
             _delta(data.today.import_kwh, y.import_kwh)),
        _row("⬆️ Export",      f"{w.export_kwh:.1f} kWh",  f"{sym}{w.export_earnings:.2f} earned"),
        _row("Cheap import",   f"{w.import_kwh_cheap:.1f} kWh",
             f"{w.cheap_import_fraction * 100:.0f}% of total import"),
        _row("Peak import",    f"{w.import_kwh_peak:.1f} kWh",
             f"{sym}{w.import_cost_peak:.2f} at peak rate", peak_style),
        _row("Import cost",    f"{sym}{w.total_import_cost:.2f}"),
        _row("Immersion saved",f"{sym}{w.immersion_savings:.2f}",
             f"{w.immersion_solar_kwh:.1f} kWh diverted", _S["positive"]),
        _row("Self-sufficiency",f"{w.self_sufficiency_pct:.1f}%", "", _S["highlight"]),

        _section("📆 Yesterday"),
        _row("Solar",   f"{y.solar_kwh:.2f} kWh"),
        _row("Import",  f"{y.import_kwh:.2f} kWh",
    ...
    if data.yesterday_forecast_accuracy_pct > 0:
        ...
        rows.append(_row("Forecast accuracy", f"{pct:.0f}%",
